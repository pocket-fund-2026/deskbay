...
const post = getPost("best-cafes-to-work-from-in-thane")!;
const AREA = AREAS["thane"];

const SCORED = cafesByArea("thane").filter((c) => c.workability !== null);
...
const faqLd = {
  ...
  mainEntity: [
    {
      "@type": "Question",
      name: "What is the best cafe to work from in Thane?",
      acceptedAnswer: {
        "@type": "Answer",
        text: `${RANKED[0].name} on ${RANKED[0].neighborhood} currently has Bombay Cafe Map's highest workability score in Thane (${RANKED[0].workability}/5), based on cited evidence about its wifi, seating and noise levels.`,
      },
    },
    {
      "@type": "Question",
      name: "How many cafes has Bombay Cafe Map scored in Thane?",
      acceptedAnswer: {
        "@type": "Answer",
        text: `${SCORED.length} of the ${cafesByArea("thane").length} Thane cafes on Bombay Cafe Map have a workability score, based on cited evidence rather than star ratings alone. See the full methodology at /about.`,
      },
    },
  ],
};
    ...
    <main className="min-h-dvh bg-ink px-6 py-10 text-paper sm:px-10">
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <p className="mt-5 text-[15px] leading-relaxed text-paper/75">
          {AREA.description} Bombay Cafe Map has scored {SCORED.length} Thane cafes on the same
          nine-factor workability model used across the city: wifi, power, seating, noise, and how
          long you can actually stay. This is the top {RANKED.length}, ranked by that score, with
          every claim traceable to the cited evidence on each cafe&apos;s own page &mdash; nothing
          here is invented to fill a paragraph.
        </p>
        <p className="mt-3 text-[15px] leading-relaxed text-paper/75">
          Thane spans {AREA.streets}. It&apos;s a satellite city with a genuinely separate cafe
          scene from the western suburbs it feeds into, growing fastest along Ghodbunder Road as
          new towers bring new office crowds &mdash; the best work spots here often look nothing
          like the specialty-coffee rooms further south, and reward a longer commute if you need a
          quiet room on the wrong side of the Thane Creek.
        </p>
        ...
        <div className="mt-3 space-y-4">
          <div>
            <p className="font-medium text-[14.5px]">What is the best cafe to work from in Thane?</p>
            <p className="mt-1 text-[13.5px] leading-relaxed text-paper/60">
              ...
              highest workability score in Thane ({RANKED[0].workability}/5), based on
          ...
          <div>
            <p className="font-medium text-[14.5px]">How many cafes has Bombay Cafe Map scored in Thane?</p>
            <p className="mt-1 text-[13.5px] leading-relaxed text-paper/60">
              {SCORED.length} of the {cafesByArea("thane").length} Thane cafes on
        ...
        <div className="mt-8 rounded-xl border border-paper/10 bg-paper/[0.03] p-5">
          <p className="text-[14px] leading-relaxed text-paper/65">
            Want the full picture, including Thane cafes that scored lower and why? See{" "}
