...
import { CAFES, AREAS, cafesByArea } from "@/lib/cafes";
...
const post = getPost("best-cafes-to-work-from-in-bandra")!;
const AREA = AREAS.bandra;

const SCORED = cafesByArea("bandra").filter((c) => c.workability !== null);
...
const faqLd = {
  ...
  mainEntity: [
    {
      "@type": "Question",
      name: "What is the best cafe to work from in Bandra?",
      acceptedAnswer: {
        "@type": "Answer",
        text: `${RANKED[0].name} on ${RANKED[0].neighborhood} currently has Bombay Cafe Map's highest workability score in Bandra (${RANKED[0].workability}/5), based on cited evidence about its wifi, seating and noise levels.`,
      },
    },
    {
      "@type": "Question",
      name: "How many cafes has Bombay Cafe Map scored in Bandra?",
      acceptedAnswer: {
        "@type": "Answer",
        text: `${SCORED.length} of the ${cafesByArea("bandra").length} Bandra cafes on Bombay Cafe Map have a workability score, based on cited evidence rather than star ratings alone. See the full methodology at /about.`,
      },
    },
  ],
};
    ...
    <main className="min-h-dvh bg-ink px-6 py-10 text-paper sm:px-10">
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <p className="mt-5 text-[15px] leading-relaxed text-paper/75">
          {AREA.description} Bombay Cafe Map has scored {SCORED.length} of the{" "}
          {cafesByArea("bandra").length} Bandra cafes in our directory on the same nine-factor
          workability model used across the city: wifi, power, seating, noise, and how long you
          can actually stay. This is the top {RANKED.length}, ranked by that score, with every
          claim traceable to the cited evidence on each cafe&apos;s own page &mdash; nothing here
          is invented to fill a paragraph.
        </p>
        <p className="mt-3 text-[15px] leading-relaxed text-paper/75">
          Bandra&apos;s cafe count spans {AREA.streets}, and the neighbourhood&apos;s bungalow
          lanes mean the best work spots aren&apos;t always the loudest names on Hill Road or
          Linking Road &mdash; several of the highest scorers below are a short walk off the main
          strip.
        </p>
        ...
        <div className="mt-8 rounded-xl border border-paper/10 bg-paper/[0.03] p-5">
          <p className="text-[14px] leading-relaxed text-paper/65">
            Want the full picture, including Bandra cafes that scored lower and why? See{" "}
