  ...
  return (
    <main className="min-h-dvh bg-ink px-6 py-10 text-paper sm:px-10">
      <div className="mx-auto max-w-2xl">
        ...
        <p className="mt-3 text-[15px] leading-relaxed text-paper/65">{DESCRIPTION}</p>
        ...
        <div className="mt-8 space-y-4">
          {BLOG_POSTS.map((post) => (
            ...
            >
              ...
              <p className="wa-mono text-paper/65">
                {post.date} · {post.readingTime}
              </p>
              <h2 className="font-display mt-1.5 text-[19px] font-medium leading-snug">
                {post.title}
              </h2>
              <p className="mt-2 text-[13.5px] leading-relaxed text-paper/65">{post.description}</p>
            </Link>
          ))}
        </div>
      </div>
